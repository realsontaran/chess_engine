...
const int AttackTables::rook_relevant_bits[] = {
...
U64 AttackTables::find_magic_number(Bitboard::square sq, int relevant_bits,
    ...
    U64 attack_mask =
        bishop ? bishop_attack_masking(sq) : rook_attack_masking(sq);
    ...
    for (int index = 0; index < occupancy_indicies; index++) {
        // init occupancies
        occupancies[index] =
            set_occupancy_map(index, relevant_bits, attack_mask);

        // init attacks
        attacks[index] = bishop
                             ? generate_bishop_attacks(sq, occupancies[index])
                             : generate_rook_attacks(sq, occupancies[index]);
    }
